		...
		scenario.steps.forEach(step => {
			if (step.embeddings !== undefined) {
				...
				step.embeddings.forEach((embedding, embeddingIndex) => {
					...
					} else if (embedding.mime_type === 'image/gif' || (embedding.media && embedding.media.type === 'image/gif')) {
						step.image = (step.image ? step.image : []).concat(['data:image/gif;base64,' + embedding.data]);
						step.embeddings[embeddingIndex] = {};
					} else {
